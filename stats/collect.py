...
#поэтому все ошибки записи глушатся и логируются.
import logging
import time
...
async def _insert(sql: str, params: tuple):
    """Выполняет запись в БД; ошибки фиксируются и не пробрасываются."""
    try:
        async with aiosqlite.connect(database.DB_NAME) as db:
            await db.execute(sql, params)
            await db.commit()
    except Exception as e:
        logger.warning("Ошибка записи статистики: %s", e)
...
async def record_message(chat_id: int, user_id=None, user_name=None, is_command=False):
    ...
    await _insert(
        "INSERT INTO stats_messages (ts, chat_id, user_id, user_name, is_command) "
        "VALUES (?, ?, ?, ?, ?)",
...
async def record_video(chat_id: int, platform: str, status: str, user_id=None, user_name=None):
    ...
    await _insert(
        "INSERT INTO stats_videos (ts, chat_id, user_id, user_name, platform, status) "
        "VALUES (?, ?, ?, ?, ?, ?)",
...
async def record_llm_usage(chat_id, tag: str, prompt_tokens: int, completion_tokens: int):
    ...
    await _insert(
        "INSERT INTO stats_llm_usage (ts, chat_id, tag, prompt_tokens, completion_tokens) "
        "VALUES (?, ?, ?, ?, ?)",
