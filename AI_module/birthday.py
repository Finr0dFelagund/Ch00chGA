...
async def birthday_text(chat_id: int, *, user_name: str, age: int | None = None) -> str:
    ...
    age_hint = f"Сегодня имениннику исполняется {age}." if age is not None else "Возраст неизвестен — не упоминай его."
    user_prompt = prompts.render("birthday", user_name=user_name, age_hint=age_hint)
    messages = [
        {"role": "system", "content": f"Ты — участник группового чата.\n\nЛичность чата:\n{personality}"},
